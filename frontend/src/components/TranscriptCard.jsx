...
export default function TranscriptCard({
    ...
    return (
        <div className="container p-0 d-flex flex-column bg-body-secondary transcript-card">
            <div className="py-2 overflow-y-scroll min-scrollbar d-flex flex-column gap-2 flex-grow-1 ">
                {snippets.map((s, i) => 
                    <div className="bg-body-tertiary shadow-sm rounded rounded-2 mx-2 p-3 d-flex justify-content-between transcript-item"
                        key={i}
                        onClick={() => onItemClick?.(timeStrToSeconds(s.time))}
                    >
                        <span className="text-start text-body-tertiary text-truncate me-1 transcript-time">{s.time}</span>
                        <span className="text-start text-body-secondary transcript-text">{s.content}</span>
                    </div>
                )}
            </div>
        </div>
